...
| Soroban contracts | Six canonical contracts in `coverfi-contracts`: Protection Engine for position rules, Protected Balance Vault for principal custody, Reserve Vault for premiums, reserves, locked capacity, and claim allocation, Oracle Adapter for prices and timestamps, Username Registry for payment identity, and Receipt Registry for proofs, access, and disputes. | Protection rules are enforced by contracts, not hidden in a private server. |
...
The canonical `coverfi-contracts` workspace is the source of truth for the six-contract Soroban architecture: Protection Engine, Protected Balance Vault, Reserve Vault, Oracle Adapter, Username Registry, and Receipt Registry. Application and backend work should align to that contract interface before public mainnet use.

Important gaps to close:

1. Generate typed clients from the canonical contract specifications.
2. Fix protection creation so the application uses the actual contract arguments and contract-derived quote.
3. Make claim status come from contract and ingestion state, not only local application state.
4. Create real receipt hashes and non-empty encrypted metadata locations before receipt anchoring.
5. Make username ownership Soroban-authoritative, with indexed search for usability.
6. Add real oracle, reserve, legal, and ingestion status endpoints.
7. Implement durable ledger ingestion and reconciliation.
8. Separate governance, oracle publishing, reserve funding, and operational signing roles.

**Recommended roadmap**

| Phase | Work |
| --- | --- |
| Canonical testnet coherence | Generate typed clients from the six contracts; fix protection creation, fee display, claim flow, principal withdrawal, usernames, and receipt anchoring; add durable ingestion and signed wallet sessions; publish testnet status, reserve state, oracle freshness, and ingestion health. |
